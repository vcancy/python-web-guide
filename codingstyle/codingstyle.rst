...
- 超长函数，没有复用和拆分，抱歉我智商低，不能理解好几屏都翻不完的，见谅。这么长居然还tm能工作，牛逼(我发现越是新手写的代码越难理解,我实习那会总被说代码写得像面条)。控制复杂度，程序的复杂性决定了一个人要花多大努力才能理解程序。Dijkstra说过『一个聪明的程序员总是清楚地知道自己的脑力容量有限，因此他得十分小心谨慎地完成编程任务』。这不意味着为了处理复杂问题你得增大你的脑力，而是说你得想尽办法尽可能降低复杂性(彻底理解你要解决的问题)。要认识到人的脑力负荷是有限的。
...
- 含义精确。不要使用诸如data，info，result，handle，process等概念太广泛的词汇给变量命名，不要使用偏门的简写，为了代码可读性冗余一些都可以(实际上对于现代语言长命名有一定好处，能减少冲突)。模棱两可的命名往往代表着某种警告（比如内聚不合理）
...
警惕某种程序猿
--------------------------------------
当然我上边废了这么多话，你可能心想着：老板又不管你代码写成啥个鸟样子，早点干完活回家不就得了。想的忒对，一般碰见这种程序员比较多的公司，先建议你扑腾一阵，如果实在束手无策又没啥话语权的时候，咱就只能：收拾包袱，让别人擦屁股吧。
