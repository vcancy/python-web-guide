...
- 对于复杂的数据结构，可以适当注释出类型，比如最新的 tornado 源码里出现了这种注释 ` __impl_kwargs = None  # type: Dict[str, Any]` 。python3 实际上可以加上类型注解了，鉴于目前 python3 的普及程度，估计也没啥用武之地了。
...
- 认识和熟悉所在团队中的成员（笔者之前一直做得不够好，这一条远比想象中重要），良好的沟通和协调能力能帮助你更快完成任务。
