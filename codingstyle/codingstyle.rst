...
运行时错误(Runtime Error): 比如文件不存在，网络请求失败等会抛出异常，这种错误需要程序员有意识进行处理，而不能假设操作一定就是成功的。
